...
void Server::takePass(Client &client, const std::string &arg)
{
	if (client.has_Pass)
	{
		std::cout << "PASS has already been sent by FD " << client.getFd() << std::endl;
		return ;
	}
	if (arg.empty())
	{
	...
	std::cout << "PASS OK pour FD " << client.getFd() << std::endl;
}
...
void Server::takeNick(Client &client, const std::string &arg)
{
	if (arg.empty())
	{
		std::cout << "NICK vide pour FD " << client.getFd() << std::endl;
		return ;
	}
	client.setNickname(arg);
	client.has_Nick = true;
	std::cout << "NICK define : " << arg << " for FD " << client.getFd() << std::endl;
...
void Server::takeUser(Client &client, const std::string &arg)
{
	if (client.has_User)
	{
		std::cout << "USER already been for FD " << client.getFd() << std::endl;
		return ;
	}
	if (arg.empty())
	{
	...
	std::cout << "USER define : " << username << " (" << realname << ") pour FD " << client.getFd() << std::endl;
	if (client.has_Pass && client.has_Nick && client.has_User && !client.regist)
		regist_Client(client);
}
...
void Server::regist_Client(Client &client)
{
	...
	std::cout << "Client FD " << client.getFd() << " register !" << std::endl;
}
